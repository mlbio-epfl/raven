...
def get_train_args(args: Optional[Dict[str, Any]] = None) -> _TRAIN_CLS:
    ...
    seed = torch.tensor([random.randint(0,100000)],dtype=torch.int64,device=torch.device(f'cuda:{torch.distributed.get_rank()}')) #if training_args.seed==0 else training_args.seed
    tensor_list = [torch.zeros(1, dtype=torch.int64, device=torch.device(f'cuda:{torch.distributed.get_rank()}')) for _ in range(torch.distributed.get_world_size())]
    torch.distributed.all_gather(tensor_list, seed)

    transformers.set_seed(seed.item())
