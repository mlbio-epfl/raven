...
@dataclass
class EnsembleDataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    r"""
    Data collator for pairwise data.
    """
    ...
    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, "torch.Tensor"]:
        ...
        for key in ("chosen", "rejected"):
            for feature in features:
                ...
                if 'weak_logits' in feature:
                    target_feature["weak_logits"] = feature["weak_logits"]
                concatenated_features.append(target_feature)

        return super().__call__(concatenated_features)

@dataclass
class TextClassificationDataCollator(DataCollatorWithPadding):
    linear_probe: bool = False
    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, "torch.Tensor"]:
        target_feature ={}
        #for key, value in features.items():  # cast data dtype for paligemma
        target_feature['input_ids'] = torch.tensor(self.tokenizer([f["txt"] for f in features], padding=True, truncation=True)['input_ids'])
        if "logits" in features[0]:
            target_feature['logits'] = torch.tensor([f["logits"] for f in features])
        try:
            target_feature['label'] = torch.tensor([f["hard_label"] for f in features], dtype=torch.long)
        except:
            target_feature['label'] = torch.tensor([f["gt_label"] for f in features], dtype=torch.long)
        
        if self.linear_probe:
            target_feature['feature'] = torch.tensor([f["feature"] for f in features])
            if 'weak_feature' in features[0]:
                target_feature['weak_feature'] = torch.tensor([f["weak_feature"] for f in features])
        #    features[key] = value.to(self.compute_dtype)
