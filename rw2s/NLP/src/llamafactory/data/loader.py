...
def _load_single_dataset(
    ...
    if dataset_attr.load_from == "ms_hub":
        ...
        logger.info_rank0(f'single dataset############ {dataset}')
...
def _get_merged_dataset(
    ...
    data_args: "DataArguments",
    training_args: "Seq2SeqTrainingArguments",
    stage: Literal["pt", "sft", "rm", "ppo", "kto", "tc"],
    ...
    logger.info_rank0(f"in_get_merged############### {datasets}")
...
def _get_preprocessed_dataset(
    ...
    logger.info_rank0('before map#########', dataset)
    dataset = dataset.map(
        preprocess_func,
        batched=True,
...
def get_dataset(
    ...
    with training_args.main_process_first(desc="load dataset"):
        logger.info_rank0('get_merged############', data_args.dataset, data_args , stage)
    ...
    with training_args.main_process_first(desc="pre-process dataset"):
        ...
        if "train" in dataset_dict:
            ...
            if easy_hard > 0:
                logger.info_rank0('Easy Hard Sampling##############################')
                print(train_dataset)
                train_dataset=easy_hard_sample(train_dataset)
...
def easy_hard_sample(dataset, shuffle=True):
    easy = []
    hard = []
    for row_data in dataset:
        row = torch.tensor(row_data['weak_logits'])
        ...
        if all_greater or all_smaller:
            easy.append(row_data)
        else:
            hard.append(row_data)
    if shuffle:
        random.seed(0)
        random.shuffle(easy)
        random.shuffle(hard)
    easyhard=easy+hard
    return datasets.Dataset.from_list(easyhard)